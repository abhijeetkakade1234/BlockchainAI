...
async function loadSqlite3IfNotLoaded(): Promise<void> {
	...
	try {
		...
		if (
			...
			SqliteDatabaseConstructor = (sqlite3Module as { Database: unknown })
				.Database;
		} else {
			console.warn(
				"SQLite3 Database constructor not found on the imported module.",
			);
		...
		if (SqliteDatabaseConstructor) {
			console.log(
				"SQLite3 module and Database constructor loaded successfully.",
			);
		} else if (sqlite3Module) {
			console.warn(
				"SQLite3 module loaded, but Database constructor could not be identified.",
			);
		}
	} catch (e) {
		console.warn(
...
async function getKeystoreDB(): Promise<any | null> {
	...
	if (!sqlite3Module || !SqliteDatabaseConstructor) {
		...
		if (!sqlite3Module || !SqliteDatabaseConstructor) {
			console.warn(
				"SQLite3 not available after load attempt. Cannot get DB instance.",
			);
			return null;
		}
	}
	...
	// biome-ignore lint/suspicious/noExplicitAny: <explanation>
	dbInitializationPromiseInternal = new Promise<any | null>(
		(resolve, reject) => {
			...
			try {
				const newDb = new DbConstructor(DB_FILE_PATH, (err: Error | null) => {
					if (err) {
						console.error(
							`Error opening database at ${DB_FILE_PATH}:`,
							err.message,
						);
						dbInitializationPromiseInternal = null;
						return reject(err);
          ...
          private_key TEXT NOT NULL
						...
						(createTableErr: Error | null) => {
							if (createTableErr) {
								console.error(
									"Error creating keystore table:",
									createTableErr.message,
								);
	...
	return new Promise<boolean>((resolve, reject) => {
		...
		db.run(
			...
			function (this: SqliteRunCallbackContext, err: Error | null) {
				...
				console.log(
					`Key for address ${address} stored/updated. Rows: ${this.changes}`,
				);
				resolve(true);
			},
		);
...
export async function getStoredPrivateKeyByAddress(
	address: string,
	...
	return new Promise<`0x${string}` | null>((resolve, reject) => {
		...
		db.get(
			sql,
			[address],
			(err: Error | null, row: { private_key: `0x${string}` } | undefined) => {
				if (err) {
